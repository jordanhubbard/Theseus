...
### Added
- ADR 0004 dual-generation for 25 Python families (`base64`, `binascii`, `bisect`, `calendar`, `colorsys`, `copy`, `difflib`, `fnmatch`, `fractions`, `glob`, `hashlib`, `heapq`, `hmac`, `html`, `json`, `keyword`, `operator`, `pprint`, `quopri`, `shlex`, `statistics`, `string`, `struct`, `textwrap`, `urllib_parse`): two empty-workspace implementations each, different hashes, both passing the public oracle, the held-out oracle, and the held-out guard. The numeric kill gate is no longer fired (25/25). ADR 0005 is not superseded, so the product stays characterization.

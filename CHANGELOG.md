...
### Added
- Characterization pass 2026-09-20: remaining high-feasibility `python_module` families plus installed npm `ulid` now have `gold/<family>/` authority + ledger + probes (`copy`, `heapq`, `reprlib`, `statistics`, `enum`, `textwrap`, `weakref`, `html_entities`, `html_parser`, `idna`, `tomlkit`, `colorsys`, `copyreg`, `genericpath`, `glob`, `pathspec`, `quopri`, `uu`, `ulid`). Enum/weakref/glob/quopri public oracles repaired so they run under `characterize.py`. `uu` is skipped on Python 3.13+. Not qualified.

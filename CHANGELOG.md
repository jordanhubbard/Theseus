...
### Added
- ADR 0004 dual-generation for 12 Python families (`base64`, `binascii`, `bisect`, `colorsys`, `difflib`, `fnmatch`, `hashlib`, `hmac`, `json`, `shlex`, `struct`, `urllib_parse`): two empty-workspace implementations each, different hashes, both passing the public oracle, the held-out oracle, and the held-out guard. The numeric kill gate is no longer fired (12/12). ADR 0005 is not superseded, so the product stays characterization.

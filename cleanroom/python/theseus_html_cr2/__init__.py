# Clean-room implementation of HTML unescaping
# No imports of html, html.parser, or any third-party libraries

import re

# Named HTML entities mapping
_NAMED_ENTITIES = {
    'lt': '<',
    'gt': '>',
    'amp': '&',
    'quot': '"',
    'apos': "'",
    'nbsp': '\u00a0',
    'iexcl': '\u00a1',
    'cent': '\u00a2',
    'pound': '\u00a3',
    'curren': '\u00a4',
    'yen': '\u00a5',
    'brvbar': '\u00a6',
    'sect': '\u00a7',
    'uml': '\u00a8',
    'copy': '\u00a9',
    'ordf': '\u00aa',
    'laquo': '\u00ab',
    'not': '\u00ac',
    'shy': '\u00ad',
    'reg': '\u00ae',
    'macr': '\u00af',
    'deg': '\u00b0',
    'plusmn': '\u00b1',
    'sup2': '\u00b2',
    'sup3': '\u00b3',
    'acute': '\u00b4',
    'micro': '\u00b5',
    'para': '\u00b6',
    'middot': '\u00b7',
    'cedil': '\u00b8',
    'sup1': '\u00b9',
    'ordm': '\u00ba',
    'raquo': '\u00bb',
    'frac14': '\u00bc',
    'frac12': '\u00bd',
    'frac34': '\u00be',
    'iquest': '\u00bf',
    'Agrave': '\u00c0',
    'Aacute': '\u00c1',
    'Acirc': '\u00c2',
    'Atilde': '\u00c3',
    'Auml': '\u00c4',
    'Aring': '\u00c5',
    'AElig': '\u00c6',
    'Ccedil': '\u00c7',
    'Egrave': '\u00c8',
    'Eacute': '\u00c9',
    'Ecirc': '\u00ca',
    'Euml': '\u00cb',
    'Igrave': '\u00cc',
    'Iacute': '\u00cd',
    'Icirc': '\u00ce',
    'Iuml': '\u00cf',
    'ETH': '\u00d0',
    'Ntilde': '\u00d1',
    'Ograve': '\u00d2',
    'Oacute': '\u00d3',
    'Ocirc': '\u00d4',
    'Otilde': '\u00d5',
    'Ouml': '\u00d6',
    'times': '\u00d7',
    'Oslash': '\u00d8',
    'Ugrave': '\u00d9',
    'Uacute': '\u00da',
    'Ucirc': '\u00db',
    'Uuml': '\u00dc',
    'Yacute': '\u00dd',
    'THORN': '\u00de',
    'szlig': '\u00df',
    'agrave': '\u00e0',
    'aacute': '\u00e1',
    'acirc': '\u00e2',
    'atilde': '\u00e3',
    'auml': '\u00e4',
    'aring': '\u00e5',
    'aelig': '\u00e6',
    'ccedil': '\u00e7',
    'egrave': '\u00e8',
    'eacute': '\u00e9',
    'ecirc': '\u00ea',
    'euml': '\u00eb',
    'igrave': '\u00ec',
    'iacute': '\u00ed',
    'icirc': '\u00ee',
    'iuml': '\u00ef',
    'eth': '\u00f0',
    'ntilde': '\u00f1',
    'ograve': '\u00f2',
    'oacute': '\u00f3',
    'ocirc': '\u00f4',
    'otilde': '\u00f5',
    'ouml': '\u00f6',
    'divide': '\u00f7',
    'oslash': '\u00f8',
    'ugrave': '\u00f9',
    'uacute': '\u00fa',
    'ucirc': '\u00fb',
    'uuml': '\u00fc',
    'yacute': '\u00fd',
    'thorn': '\u00fe',
    'yuml': '\u00ff',
}

_ENTITY_PATTERN = re.compile(r'&(?:#([0-9]+)|#[xX]([0-9a-fA-F]+)|([a-zA-Z][a-zA-Z0-9]*));')


def unescape(s):
    """Replace HTML entities in string s with their corresponding characters."""
    def replace_entity(match):
        decimal = match.group(1)
        hexadecimal = match.group(2)
        named = match.group(3)

        if decimal is not None:
            # Decimal numeric entity: &#65;
            try:
                codepoint = int(decimal, 10)
                return chr(codepoint)
            except (ValueError, OverflowError):
                return match.group(0)
        elif hexadecimal is not None:
            # Hexadecimal numeric entity: &#x41;
            try:
                codepoint = int(hexadecimal, 16)
                return chr(codepoint)
            except (ValueError, OverflowError):
                return match.group(0)
        elif named is not None:
            # Named entity: &lt;
            if named in _NAMED_ENTITIES:
                return _NAMED_ENTITIES[named]
            else:
                return match.group(0)
        else:
            return match.group(0)

    return _ENTITY_PATTERN.sub(replace_entity, s)


def html2_unescape_lt():
    return unescape('&lt;')


def html2_unescape_amp():
    return unescape('&amp;')


def html2_unescape_quot():
    return unescape('&quot;')
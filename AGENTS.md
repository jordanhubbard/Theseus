...
Given a behavioral spec with a `python_cleanroom` backend, synthesizes a complete reimplementation of the package from scratch — no importing the original, no cross-language wrappers. `theseus_registry.json` currently contains 360 verified Python packages plus 32 policy-failed entries quarantined by clean-room import checks. See `docs/architecture.md §Layer 3` and `docs/cleanroom-spec-format.md`.

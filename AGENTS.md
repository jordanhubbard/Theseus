...
Given a behavioral spec with a `python_cleanroom` backend, historically synthesized a reimplementation without importing the original. `theseus_registry.json` lists 396 packages with `status=verified`; that bit is **legacy isolation only**. **12 packages are qualified** (`theseus_base64_q`, `theseus_fnmatch_q`, `theseus_json`, `theseus_shlex_q`, `theseus_binascii_q`, `theseus_difflib_q`, `theseus_hashlib_q`, `theseus_hmac_q`, `theseus_struct_q`, `theseus_urllib_parse_q`, `theseus_bisect_q`, `theseus_colorsys_q`) by ADR 0004 dual-generation receipts. 12 of 12 attempts qualified, so the numeric kill gate is no longer fired. ADR 0005 is not superseded, so the product stays characterization. Gold-set families have reviewed characterization ledgers (ADR 0003) and characterization records (ADR 0006). See [ADR 0001](docs/decisions/0001-verification-ladder.md), [ADR 0004](docs/decisions/0004-qualification-protocol.md), and `reports/audit/corpus-autopsy.md`.
...
gold/                     Characterization authorities (qualification gold set + characterization cohort)
  ...
  json/held_out.zspec.zsdl  Held-out oracle (synthesizer must never see this)
...
theseus_registry.json     Registry of clean-room packages and verification status
reports/synthesis/
...
**Grade the public API.** Gold-set specs call `dumps` / `loads` (or the real export names) with arguments. Do not add zero-argument self-test wrappers (`json_loads_int`). `cleanroom_verify.py` calls `fn(*args, **kwargs)` and supports `python_call_raises`.

Held-out vectors live outside `zspecs/` (see `gold/json/held_out.zspec.zsdl`) so wave synthesis never ingests them. `tools/held_out_guard.py` fails if distinctive held-out tokens appear in a synthesis prompt.

Legacy `theseus_*` factory specs still use zero-arg wrappers; they are not the template for new gold-set work.

```yaml
# Gold-set (theseus_json)
function: dumps
args: [[1, 2, 3]]
...
```python
# WRONG — self-test wrapper, not the public API
def json_loads_int():
    return loads('{"a": 1}')["a"]

# RIGHT — export dumps/loads; the oracle calls them with arguments
...
# 5. Check for failures; gold-set specs must call the public API with args
...
invariant theseus_mylib.foo:
  ...
  category: basic
  kind: python_call_eq
  function: foo                  # public export; harness calls fn(*args, **kwargs)
  args: ["input"]
  expected: "expected"
```

The `provenance.notes` list is passed to the synthesis LLM as context. Name the public exports, not wrapper helpers. For gold-set work also write `gold/<family>/package.md` and a held-out oracle outside `zspecs/`.
...
from theseus_json import dumps, loads
print(loads(dumps({'a': 1})))
"
```
...
| Error | Cause | Fix |
|-------|-------|-----|
| `TypeError: fn() missing 1 required positional argument` | Spec still uses a wrapper name, or gold-set spec forgot `args` | Call the public API with arguments (gold-set); only legacy factory specs use zero-arg wrappers |
...
# Gold-set characterization (ADR 0003) — public docs + live probes, not implementation source
...
| `docs/decisions/0003-characterization-loop.md` | Characterization loop, live probes, uncertainty ledgers |
| `docs/characterization-loop.md` | How to run the gold-set characterization loop |

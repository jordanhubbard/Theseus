...
1,934 machine-readable behavioral specs (ZSDL files) — 729 of them target npm packages — covering Python stdlib, npm, ctypes C libraries (incl. libpcap + pcapng), and Rust PyO3 extension modules. Each spec defines invariants verified against the real installed library. ZSDL gained five new spec kinds across batches 112–115 + the libpcap challenge: `node_chain_eq` (fluent builder chains), `node_property_eq` (sugar for post-construction property reads), `node_sandbox_chain_eq` (chain in a tempdir cwd, for fs packages), `ctypes_chain_eq` (handle-threading C APIs), `ctypes_sandbox_chain_eq` (ctypes chain + binary-blob seeded tempdir, for libpcap/pcapng). See `docs/architecture.md §Layer 2` and `docs/writing-specs.md`.
...
Current spec counts by backend: node (729), rust_module (479), python_cleanroom (390), python_module (318), ctypes (12 — incl. libpcap + pcapng), cli (4), node_cleanroom (1).
